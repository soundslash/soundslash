...
class StartEncodedStreaming(Base):

    def __init__(self, stream_manager, pipeline_server_id, stream):
        super(StartEncodedStreaming, self).__init__()
    ...
    def run(self, respond):
        ...
        if response is None:
            ...
            if not "error" in server:
                player = EncodedStreamer(self.stream, server["db_obj"])
                player.scheduler = EncodedScheduler(player)
